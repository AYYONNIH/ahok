...
zeroyt7.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && !mem.includes(zeroyt7.user.jid)) {
        ...
        wel = `Halo @${anu_user} \nWelcome In ${mdata.subject} \nJangan Lupa Follow Sosial Media Owner Ku Github : https://github.com/Ayyonnih`
        ...
        sendButImage(mdata.id, wel, "©Created :Ayyon Bot", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(zeroyt7.user.jid)) {
        ...
        sendButImage(mdata.id, out, "©Created :Ayyon Bot", buff, but)
      }
